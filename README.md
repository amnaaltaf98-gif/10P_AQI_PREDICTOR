...
Hopsworks Feature Store
        |
train.py  (Ridge / RandomForest / XGBoost / LightGBM, compared against a persistence baseline)
        |
...
2. Copy `.env.example` to `.env` and fill in:
   ...
   - `HOPSWORKS_API_KEY` and `HOPSWORKS_PROJECT_NAME` (create a free project at hopsworks.ai)
...
cd ../fastapi
uvicorn main:app --reload        # prediction API on localhost:8000
